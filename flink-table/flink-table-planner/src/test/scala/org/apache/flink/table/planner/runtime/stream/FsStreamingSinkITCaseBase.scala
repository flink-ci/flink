    ...
    val ddl =
      s"""
        ...
        if (timeExtractorFormatterPattern.nonEmpty) s" '" +
          s"${PARTITION_TIME_EXTRACTOR_TIMESTAMP_FORMATTER.key()}' = '$timeExtractorFormatterPattern',"
        else ""
      }
         | 
...
class FiniteTestSource(elements: Iterable[Row], watermarkGenerator: Row => Long) extends SourceFunction[Row]
