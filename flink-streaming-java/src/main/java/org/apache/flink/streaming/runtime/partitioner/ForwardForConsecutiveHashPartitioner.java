...
/**
 * If there are multiple consecutive the same groupBy(i.e. keyBy), SQL planner will change them
 * except the first one to use forward partitioner, so that these operators can be chained to reduce
 ...
 * optimizes the case of multiple consecutive the same groupBy, it should use this partitioner, and
 * then the runtime framework will change it to forward/hash after the operator chain creation.
 *
 * <pre>{@code
 * A --[hash]--> B --[hash]--> C
 *            |
 ...
 *
 * <p>This partitioner should only be used for SQL Batch jobs and use Adaptive Batch Scheduler.
 *
