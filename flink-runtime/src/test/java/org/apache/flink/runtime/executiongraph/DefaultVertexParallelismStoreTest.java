...
public class DefaultVertexParallelismStoreTest extends TestLogger {
    @Test
    public void testEmptyStoreGet() {
        DefaultVertexParallelismStore store = new DefaultVertexParallelismStore();

        Optional<VertexParallelismInformation> optInfo =
                store.getParallelismInfo(new JobVertexID());

        Assert.assertFalse(optInfo.isPresent());
    }
    ...
    public void testSetInfo() {
        ...
        Optional<VertexParallelismInformation> optInfo = store.getParallelismInfo(id);

        Assert.assertTrue(optInfo.isPresent());
        Assert.assertEquals(info, optInfo.get());
    }
