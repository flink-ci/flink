...
public class SchedulerBaseComputeVertexParallelismTest extends TestLogger {
    ...
    public void testMaxParallelismDefaulting() {
        ...
        Optional<VertexParallelismInformation> infoOpt =
                store.getParallelismInfo(jobVertex.getID());

        Assert.assertTrue("contains parallelism info for vertex", infoOpt.isPresent());

        VertexParallelismInformation info = infoOpt.get();
